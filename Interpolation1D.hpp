...
public:
  ...
  Interpolation1D(const std::vector<double>& x, const std::vector<T>& y,
    ...
    : m_interp(interp), m_ydef(ydef), m_xy(std::min(x.size(),y.size()))
  {
    for(unsigned ixy=0;ixy<m_xy.size();ixy++)
      m_xy[ixy] = std::make_pair<double, T>(x[ixy],y[ixy]);
  ...
  class BinAddOp { public: T operator() (double x, const T& y1, const T& y2) 
    { return y1+y2; } };
  class BinSubOp { public: T operator() (double x, const T& y1, const T& y2)
    { return y1-y2; } };
  class BinMulOp { public: T operator() (double x, const T& y1, const T& y2)
    { return y1*y2; } };
  class BinDivOp { public: T operator() (double x, const T& y1, const T& y2)
    { return y1/y2; } };
  ...
  class UnNegOp { public: T operator() (double x, const T& y) 
    { return -y; } };
  class UnExpOp { public: T operator() (double x, const T& y) 
    { return std::exp(y); } };

  void insert(double x, const T& y)
  {
    m_xy.push_back(std::make_pair<double, T>(x,y));
    std::sort(m_xy.begin(), m_xy.end());
  }
  ...
  T y(double x) const
  {
    if(m_xy.size() == 0)return m_ydef;
    else if(m_xy.size() == 1)m_xy.front().second;

    typename xy_vec_type::const_iterator xy1 =
      std::upper_bound(m_xy.begin(), m_xy.end(), 
		       std::make_pair<double, T>(x, T()));
  ...
  {
    assert(xlo <= xhi);
    if(m_xy.size() == 0)return m_ydef * (xhi-xlo);
    else if(m_xy.size() == 1)return m_xy.front().second * (xhi-xlo);
    T sum = T();
    double x0 = xlo;
    T y0 = y(x0);
...
#endif // not defined INTERPOLATION1D_HPP
